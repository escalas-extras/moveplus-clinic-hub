...
function Dashboard() {
  ...
  const stats = useQuery({
    ...
    queryFn: async () => {
      const [pacientes, novos, hoje, mesPag, sessoesMes, pendentes, reavalAtrasadas, altasMes, sem30d] = await Promise.all([
...
function StatCard({ icon: Icon, label, value, hint, to, color }: { icon: any; label: string; value: any; hint?: string; to?: string; color?: string }) {
  const content = (
    <Card className={cn("p-4 sm:p-5 h-full", to && "hover:shadow-md transition-all cursor-pointer hover:-translate-y-0.5")}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="rounded-lg p-2 flex-shrink-0" style={{ background: color ? `${color}15` : undefined, color }}>
