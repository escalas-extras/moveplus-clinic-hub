import { createFileRoute, Link, useNavigate } from "@tanstack/react-router";
import { useState } from "react";
import { Settings2, Wallet } from "lucide-react";
import { useActiveClinic } from "@/lib/active-clinic";
import { AppShell, OutlineActionButton } from "@/components/layout";
import { OpsModuleStack, OpsPageHero } from "@/components/ops";
import {
  FinanceDashboardPanel,
  FinanceOperationsGrid,
  FinanceFiltersBar,
  FINANCE_ROUTES,
} from "@/components/finance";
import { defaultDashboardFilters } from "@/lib/finance";

export const Route = createFileRoute("/_authenticated/app/financeiro/")({
  component: FinanceiroHomePage,
});

function FinanceiroHomePage() {
  const { clinicId, loading: clinicLoading } = useActiveClinic();
  const [dashboardFilters, setDashboardFilters] = useState(() => defaultDashboardFilters());
  const navigate = useNavigate();

  return (
    <AppShell clinical>
      <OpsPageHero
        icon={Wallet}
        eyebrow="Gestão"
        title="Financeiro"
        description="Como está minha clínica? Indicadores, rankings e acesso rápido aos módulos operacionais."
        breadcrumbs={[{ label: "Clínica", to: "/app" }, { label: "Financeiro" }]}
        actions={
          <OutlineActionButton asChild className="h-9 px-3 text-xs">
            <Link to={FINANCE_ROUTES.administracao}>
              <Settings2 className="mr-1.5 h-3.5 w-3.5" />
              Administração
            </Link>
          </OutlineActionButton>
        }
      />

      <OpsModuleStack>
        <FinanceOperationsGrid
          clinicId={clinicId}
          clinicLoading={clinicLoading}
          dashboardFilters={dashboardFilters}
        />

        <FinanceFiltersBar
          clinicId={clinicId}
          filters={dashboardFilters}
          onFiltersChange={setDashboardFilters}
        />

        <FinanceDashboardPanel
          clinicId={clinicId}
          clinicLoading={clinicLoading}
          filters={dashboardFilters}
          onNewReceivable={() => navigate({ to: FINANCE_ROUTES.receber })}
        />
      </OpsModuleStack>
    </AppShell>
  );
}
